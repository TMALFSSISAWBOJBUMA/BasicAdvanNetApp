...
Network.prototype.putRequest = function (url, body) {
  return new Promise((resolve, reject) => {
    ...
    xhttp.onerror = (error) =>
      reject("Network error: cannot perform PUT request: " + error);
    xhttp.send(body);
  });
};
...
Network.prototype.getDeviceId = function (readerIP) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      var deviceID = this.controller.getXMLTagValue(
        xml,
        "/response/data/devices/device/id",
      )[0];
...
Network.prototype.getRFData = function (readerIP) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      var address =
        "http://" + readerIP + ":3161/devices/" + deviceID + "/reader";
...
Network.prototype.getReaderVolume = function (readerIP) {
  return new Promise(async (resolve, reject) => {
    try {
      var address =
        "http://" + readerIP + ":3161/devices/" + this.deviceID + "/actions";
      var xml = await this.getRequest(address);
      var volume = this.controller.getXMLTagValue(
        xml,
        "/response/data/entries/entry/volume",
      )[0];
      resolve(volume);
    } catch (error) {
      reject(error);
...
Network.prototype.getAntennas = function (readerIP) {
  return new Promise(async (resolve, reject) => {
    try {
      var address =
        "http://" + readerIP + ":3161/devices/" + this.deviceID + "/antennas";
      var xml = await this.getRequest(address);
      resolve(xml);
    } catch (error) {
      reject(error);
...
Network.prototype.getReaderStatus = function (readerIP) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      var xml = await this.getRequest(address);
      var status = this.controller.getXMLTagValue(
        xml,
...
Network.prototype.startReader = function (readerIP) {
  return new Promise(async (resolve, reject) => {
    try {
      var address =
        "http://" + readerIP + ":3161/devices/" + this.deviceID + "/start";
      var xml = await this.getRequest(address);
...
Network.prototype.stopReader = function (readerIP) {
  return new Promise(async (resolve, reject) => {
    try {
      var address =
        "http://" + readerIP + ":3161/devices/" + this.deviceID + "/stop";
      var xml = await this.getRequest(address);
      var status = this.controller.getXMLTagValue(xml, "/response/status")[0];
      if (status == "OK") resolve(status);
      else reject("Could not stop the reader properly");
    } catch (error) {
      reject(error);
    }
  });
...
Network.prototype.getInventory = function (readerIP) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      var jsonItems = JSON.parse(
        this.controller.getXMLTagValue(xml, "/response/data/result")[0],
      );
      resolve(jsonItems);
    } catch (error) {
      reject(error);
...
Network.prototype.savePower = function (readerIP, power) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      var status = this.controller.getXMLTagValue(xml, "/response/status")[0];
      if (status == "OK") resolve(status);
      else reject("Could not save power");
    } catch (error) {
      reject(error);
    }
  });
...
Network.prototype.saveSensitivity = function (readerIP, sensitivity) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      var status = this.controller.getXMLTagValue(xml, "/response/status")[0];
      if (status == "OK") resolve(status);
      else reject("Could not save sensitivity");
    } catch (error) {
      reject(error);
    }
  });
...
Network.prototype.saveAntennas = function (readerIP, activeAntennas) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (status == "OK") resolve(status);
      else reject("Could not save antennas");
    } catch (error) {
      reject(error);
    }
  });
...
Network.prototype.createAntennasXML = function (deviceID, activeAntennas) {
  ...
  for (var antenna in activeAntennas) {
    xml +=
      ...
      deviceID +
      "," +
      activeAntennas[antenna] +
      ",0,0,-1,loc_id,0,0,0</def>" + //<def>AdvanReader-m4-150,2,0,0,-1,loc_id,40,210,2</def>
      "</entry>";
  }
  xml += "</entries>" + "</request>";
  return xml;
