...
View.prototype.toggleStartStop = function (startStopButton) {
  if (!this.controller.isConnected) {
    this.displayOperationStatus(
      "alert-danger",
      "Unable to start reader. Please, specify an IP first.",
    );
    return;
  }
...
View.prototype.getActiveAntennas = function () {
  ...
  for (var i = 0; i < antennas.length; i++) {
    if (antennas[i].classList.contains("active")) activeAntennas.push(i + 1);
  }
...
View.prototype.displayAntennas = function (numPorts, activeAntennas) {
  ...
  for (var port = 1; port <= numPorts; port++) {
    ...
    html +=
      ...
      activeAntenna +
      '">' +
      '<input type="checkbox" name="options" id="antennaPort' +
      port +
      '" autocomplete="off">' +
      port +
      "</label>";
...
View.prototype.isActiveAntenna = function (port, activeAntennas) {
  for (var a in activeAntennas) {
    if (activeAntennas[a].port == port) return "btn btn-secondary active";
  }
...
View.prototype.displayReaderStatus = function (status) {
  ...
  switch (status) {
    case "RUNNING":
      ...
      saveButtonText = this.controller.isConnected
        ? "Stop the reader to save"
        : "Not connected";
      break;
    case "STOPPED":
...
View.prototype.displaySaveStatus = function (saveStatus) {
  switch (saveStatus) {
    ...
    case "noAntennas":
      this.displayOperationStatus(
        "alert-danger",
        "Please specify at least one antenna.",
      );
      break;
    default:
...
View.prototype.getValuesToSave = function () {
  if (!this.controller.isConnected) return;
...
View.prototype.increaseInputNumber = function (inputName) {
  ...
  else document.getElementById(inputName).stepUp();
};
...
View.prototype.decreaseInputNumber = function (inputName) {
  if (inputName === "sensitivity") document.getElementById(inputName).stepUp();
  else document.getElementById(inputName).stepDown();
};
